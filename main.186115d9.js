...
(function() {
var WARHOL_BG = ['#FF4DA6','#00C2C7','#FFE600','#FF6E00','#FF2A2A','#2B6BFF','#8AFF00','#A64DFF','#00E5FF','#FF8C00','#00FF87','#FF00AA','#FFC100','#3A86FF','#FF5C8A','#D90368'];
...
var bg;
var pickBg = function(seen) {
var pool = WARHOL_BG.slice();
for (var i = 0; i < seen.length; i++) pool = pool.filter(function(x) { return x !== seen[i]; });
if (!pool.length) pool = WARHOL_BG.slice();
return pool[Math.floor(Math.random() * pool.length)];
};
var seen = [];
if (r > 0) {
var up = grid.children[(r - 1) * COLS + c];
if (up) seen.push(up.getAttribute('data-bg'));
}
if (c > 0) {
var lf = grid.children[r * COLS + c - 1];
if (lf) seen.push(lf.getAttribute('data-bg'));
}
bg = pickBg(seen);
var hue = randHue([]);
var nearHue = [];
if (r > 0) {
var upCell = grid.children[(r - 1) * COLS + c];
if (upCell) nearHue.push(parseInt(upCell.getAttribute('data-hue'), 10));
}
if (c > 0) {
var lfCell = grid.children[r * COLS + c - 1];
if (lfCell) nearHue.push(parseInt(lfCell.getAttribute('data-hue'), 10));
}
hue = randHue(nearHue);
...
heroTrack.classList.remove('slide');
