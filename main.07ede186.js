...
if (rel && rel.tracks[item.trackIdx]) _preAudio.src = rel.tracks[item.trackIdx].file;
}
function npToggle() {
...
_swipe = { y: e.clientY, x: e.clientX, target: npBar };
}, { passive: true });
npBar.addEventListener('pointermove', function(e) {
if (!_swipe || _swipe.target !== npBar) return;
if (Math.abs(e.clientX - _swipe.x) > 24) { _swipe = null; return; }
if (e.clientY - _swipe.y < -40) { _swipe = null; fsOpen(); }
}, { passive: true });
npBar.addEventListener('pointerup', function() { _swipe = null; }, { passive: true });
...
_swipe = { y: e.clientY, x: e.clientX, target: fs };
}, { passive: true });
...
if (Math.abs(e.clientX - _swipe.x) > 24) { _swipe = null; return; }
if (e.clientY - _swipe.y > 48) { _swipe = null; fsClose(); }
}, { passive: true });
...
if (_fsTouch && _fsTouch.active) { _fsTouch = null; fsClose(); }
_fsTouch = null;
}, { passive: true });
...
touch devices have no hover, so a tap toggles each photo to color and
back. Only wired where the pointer is coarse, leaving desktop hover
behavior untouched. */
(function() {
