...
function cellAt(grid, r, c, COLS, ROWS) {
return (r >= 0 && c >= 0 && r < ROWS && c < COLS) ? grid.children[r*COLS+c] : null;
}
...
var n0 = cellAt(grid, r+dr, c+dc, COLS, ROWS);
if (n0) seenFam[hueFamily(n0.getAttribute('data-bg'))] = true;
}
}
pool = pool.filter(function(x) { return !seenFam[hueFamily(x)]; });
...
for (var dc = -1; dc <= 1; dc++) {
if (dr === 0 && dc === 0) continue;
var n = cellAt(grid, r+dr, c+dc, COLS, ROWS);
