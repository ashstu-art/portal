...
const LINK_SVG = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>';
...
function queueRelease(rIdx, autoplay) {
var rel = RELEASE_LIST[rIdx];
if (!rel || !rel.tracks[0]) return;
pState.releaseIdx = rIdx;
pState.playlist = rel.tracks.map(function(t, i) { return { title: t.title, file: t.file, releaseIdx: rIdx, trackIdx: i }; });
pState.plIdx = 0;
var t = rel.tracks[0];
...
if (_npArt && art) { _npArt.src = encodeURI(art); _npArt.hidden = false; }
npAudio.src = t.file; _src = t.file;
if (autoplay) npLoad(t.title, rel.name, t.file, true);
updatePlayUI();
}
function releaseHashName() {
if (!location.hash) return null;
var m = location.hash.match(/^#release\/(.*)$/);
if (!m) return null;
try { return decodeURIComponent(m[1]); } catch (e) { return null; }
}
function applyReleaseHash() {
var name = releaseHashName();
if (_hashApplied || !name || !RELEASE_LIST.length) return;
for (var i = 0; i < RELEASE_LIST.length; i++) {
if (RELEASE_LIST[i].name === name) {
_hashApplied = true;
queueRelease(i, false);
setTracklist(i, false);
var cards = document.querySelectorAll('.rel-card');
var card = cards[i];
if (card) setTimeout(function() { card.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 60);
return;
}
}
}
...
document.querySelectorAll('.rel-btn--share').forEach(function(btn) {
...
function copyReleaseLink(btn) {
var name = btn.dataset.share;
var url = location.origin + location.pathname + '#release/' + encodeURIComponent(name);
function flash() {
...
btns += '<button type="button" class="rel-btn rel-btn--share" data-share="' + escHtml(rel.name) + '" aria-label="Copy link to ' + escHtml(rel.name) + '" title="Copy link">' + LINK_SVG + '</button>';
...
if (!haveEmbedded) { renderAllContent(results, false); applyReleaseHash(); return; }
if (JSON.stringify(results) !== JSON.stringify(embedded)) { renderAllContent(results, true); applyReleaseHash(); }
});
}
loadDynamicContent();
let _lbIdx = 0;
